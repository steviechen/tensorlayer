...
def seq_minibatches(inputs, targets, batch_size, seq_length, stride=1):
    ...
    for start_idx in range(0,
                           len(inputs) - n_loads + 1, (batch_size * stride)):
        seq_inputs = np.zeros(
            (batch_size, seq_length) + inputs.shape[1:], dtype=inputs.dtype)
        seq_targets = np.zeros(
            (batch_size, seq_length) + targets.shape[1:], dtype=targets.dtype)
...
def seq_minibatches2(inputs, targets, batch_size, num_steps):
    ...
    # data = np.zeros([batch_size, batch_len])
    data = np.zeros(
        (batch_size, batch_len) + inputs.shape[1:], dtype=inputs.dtype)
