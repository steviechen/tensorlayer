...
class TaskSpecDef(object):
    ...
    def __init__(self,
        ...
        if ps_hosts and worker_hosts:
            self.ps_hosts = ps_hosts if isinstance(
                ps_hosts, list) else ps_hosts.split(',')
            self.num_ps = len(self.ps_hosts)
            self.worker_hosts = worker_hosts if isinstance(
                worker_hosts, list) else worker_hosts.split(',')
    ...
    def create_server(self):
        if self._server is None and self.ps_hosts and self.worker_hosts and not self.is_evaluator(
            ...
            self._server = tf.train.Server(
                self._cluster_spec, job_name=self.type, task_index=self._index)
            if self.is_ps():
                self._server.join()
    ...
    def user_last_worker_as_evaluator(self):
        ...
        if self.num_workers <= 1:
            raise Exception(
                'You need more than one worker instance to use one as evaluator'
            )
        return TaskSpecDef(
            type=self.type,
            index=self._index,
...
def TaskSpec():
    ...
    if 'TF_CONFIG' in os.environ:
        ...
        cluster_data = env.get('cluster', None) or {
            'ps': None,
            'worker': None,
            'master': None
        }
