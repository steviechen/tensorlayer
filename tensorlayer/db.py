...
class TensorDB(object):
    ...
    def __init__(self,
        ...
        ##
        print("[TensorDB] Connect SUCCESS {}:{} {} {} {}".format(
            ip, port, db_name, user_name, studyID))
    ...
    def save_params(self, params=[], args={}):  #, file_name='parameters'):
        ...
        s = time.time()
        f_id = self.paramsfs.put(
            self.__serialization(params))  #, file_name=file_name)
        ...
        print("[TensorDB] Save params: SUCCESS, took: {}s".format(
            round(time.time() - s, 2)))
        return f_id
    ...
    def find_one_params(self, args={}, sort=None):
        ...
        try:
            ...
            print("[TensorDB] Find one params SUCCESS, {} took: {}s".format(
                args, round(time.time() - s, 2)))
            return params, f_id
        except:
            return False, False
    ...
    def run_job(self, jid):
        self.db.JOBS.find_one_and_update({
            '_id': jid
        }, {
            '$set': {
                'Running': True,
