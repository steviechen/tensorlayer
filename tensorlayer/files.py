...
def load_mnist_dataset(shape=(-1, 784), path="data"):
    ...
    def load_mnist_images(path, filename):
        filepath = maybe_download_and_extract(
            filename, path, 'http://yann.lecun.com/exdb/mnist/')
...
def load_cifar10_dataset(
        shape=(-1, 32, 32, 3), path='data', plotable=False, second=3):
...
def load_flickr1M_dataset(tag='sky',
    ...
    print("[Flickr1M] using {}% of images = {}".format(size * 10,
                                                       size * 100000))
    images_zip = [
...
def load_voc_dataset(path='data',
    ...
    if dataset != "2012test":
        ...
        if folder_exists(os.path.join(path, extracted_filename)) is False:
            print("[VOC] {} is nonexistent in {}".format(
    ...
    if dataset != "2012test":
        ...
        print("[VOC] {} maps for semantic segmentation found".format(
        ...
        print("[VOC] {} maps for instance segmentation found".format(
            len(imgs_semseg_file_list)))
...
def save_npz(save_list=[], name='model.npz', sess=None):
    ...
    if sess:
        ...
        try:
            ...
            print(
                " Fail to save model, Hint: pass the session into this function, save_npz(network.all_params, name='model.npz', sess=sess)"
            )
...
def save_ckpt(sess=None,
    ...
    if printable:
        for idx, v in enumerate(var_list):
            print("  param {:3}: {:15}   {}".format(idx, v.name,
                                                    str(v.get_shape())))
...
def maybe_download_and_extract(filename,
    ...
    if not os.path.exists(filepath):
        ...
        print('Succesfully downloaded %s %s bytes.' %
